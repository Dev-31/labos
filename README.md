...
Run `labos release readiness` first to see the current release blockers in one machine-readable payload. It reports whether the checkout is clean, whether the optional Docker smoke can run on the current host, and includes `next_action`, `pending_steps`, and `tag_ready` fields so release automation can explain exactly what is still blocking the `v0.1.0` tag. The nested `git` object exposes git entries, git issue code, and git remediation fields, which makes an otherwise-generic dirty-tree failure actionable. The nested `docker` object also exposes `cli_path`, `daemon_error`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, and `user_in_socket_group`, which makes it obvious whether the blocker is a missing Docker binary, a daemon connectivity failure, or a permission problem talking to the daemon because the payload now shows exactly which user and socket-group mismatch is in the way.

Run `labos release evidence` when you want the release-checklist evidence template pre-filled with the current commit SHA, the standard verification commands, the docs surface to re-read, the current Git/Docker blocker detail, and the same `next_action` / `pending_steps` / `tag_ready` release-decision fields. Its `git` payload carries `entries`, `issue_code`, and `remediation`, while its `docker` payload carries `cli_path`, `daemon_error`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, and `user_in_socket_group`, so the release record preserves the exact host-side blocker plus the recommended fix.
...
Then run `labos release smoke-docker` for the runtime-specific evidence payload. It first reuses the same Docker readiness probe and only runs the real-Docker pytest smoke when the host is actually ready, returning one JSON object with the probe result and pytest output. The release helper subprocesses scrub any inherited `VIRTUAL_ENV` value before spawning nested `uv run ...` checks, which keeps cron/CI evidence clean instead of leaking unrelated-venv warnings into the JSON payloads.

Then run `labos runtime probe-docker` for the low-level runtime-specific readiness check. It exits non-zero when the Docker CLI is missing or the daemon is unreachable, and the JSON payload includes `cli_path`, `daemon_error`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, and `user_in_socket_group` when available, so release-prep automation can fail honestly before attempting the smoke test and can explain permission-denied Docker failures without extra host inspection.
