...
`readiness` reports the current Phase 18 release blockers as JSON. Today it checks whether the Git working tree is clean and whether the optional Docker runtime smoke can run on the current host, then exits non-zero while any blocker remains. The payload also includes `next_action`, `pending_steps`, and `tag_ready` so operators can see exactly what still stands between the current checkout and an honest `v0.1.0` tag. The nested `git` payload includes git entries, git issue code, and git remediation fields, while the nested `docker` payload includes `cli_path`, `daemon_error`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, and `user_in_socket_group`, so the blocker record distinguishes between specific dirty-tree paths, a missing Docker binary, a generic daemon handshake failure, and a permission-denied daemon access problem.

`evidence` emits a machine-readable version of the release-checklist evidence template. It includes the current commit SHA, the standard verification commands, the docs surface to re-read before tagging, the current Git/Docker blocker detail, and the same `next_action` / `pending_steps` / `tag_ready` fields so release notes or issue templates can be pre-filled honestly. Its nested `git` object preserves `entries`, `issue_code`, and `remediation`, and its nested `docker` object preserves `cli_path`, `daemon_error`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, and `user_in_socket_group`, for auditability.
...
`probe-docker` reports whether the optional real-Docker smoke test can run on the current host. It prints JSON with `cli_present`, `cli_path`, `daemon_reachable`, `daemon_error`, `detail`, `issue_code`, `remediation`, `current_user`, `user_groups`, `daemon_target`, `socket_path`, `socket_owner`, `socket_group`, `socket_mode`, `user_in_socket_group`, and `ready`, then exits non-zero when the environment is not actually ready.
