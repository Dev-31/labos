...
- Docker readiness payloads now include `cli_path` and `daemon_error`, so release blockers distinguish missing Docker binaries from unreachable daemons across `labos runtime probe-docker`, `labos release readiness`, and `labos release evidence`.
...
- Release-prep tooling now also includes `labos release smoke-local` and `make smoke-local`, which bootstrap a temporary local API/database/storage environment, run the docs + CLI release smokes against it, and append the current Docker smoke payload in one honest JSON report.
- Release-prep tooling now also includes `labos release smoke-docker` and `make smoke-docker`, which reuse the Docker readiness probe and only run the real-Docker pytest smoke when the current host can actually support it.
